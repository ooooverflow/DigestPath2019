...
def main(params):
	...
	if params['writer'] == True:
		writer = SummaryWriter(comment='_resnet18 4fold_{} all dataset(pos+neg) bagging lowest 10%'.format(params['test_fold']))
	...
	for epoch in range(params['max_epoch']):
		...
		torch.save(retinanet.module.state_dict(), 'ckpt/latest_resnet18_fold_{}_all_dataset_using_bagging_lowest_10.pth'.format(params['test_fold']))
		...
		if float(recall_record[-1]) > best_recall:
			...
			torch.save(retinanet.module.state_dict(),
					   'ckpt_new/best_recall_resnet18_fold_{}_all_dataset_using_bagging_lowest_10.pth'
...
if __name__ == '__main__':
	...
	params = {
		...
		'test_stride_num': 3,
		'label_csv': './bbox/retinanet_resnet_18_using_test_prediction_filtered_by_bagging_lowest_10%_for_round_1_training.csv'
	}
