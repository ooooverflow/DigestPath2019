...
pred_csv = './test_result_da/retinanet_resnet18_round4_fold_2_weight_loss_1_on_test_data_best_valid_recall_original_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3(round3)_0925.csv'
...
min_predict_bbox_on_positive_images_record = []
...
for i, (image, bbox, image_, image_name) in enumerate(tqdm(test_dataset)):
	...
	num_pred_bbox = int(torch.Tensor(pred_scores)[torch.Tensor(pred_scores) >= 0.5].size(0))

	if num_pred_bbox != 0 and len(bbox) == 0:
		if num_pred_bbox > max_predict_bbox_on_negative_images:
			max_predict_bbox_on_negative_images = num_pred_bbox
			max_name = image_name

	if len(bbox) != 0:
		min_predict_bbox_on_positive_images_record.append(num_pred_bbox)
		if num_pred_bbox < min_predict_bbox_on_positive_images:
			min_predict_bbox_on_positive_images = num_pred_bbox
			min_name = image_name

	# print(image_name, num_pred_bbox, len(bbox))
...
print(pred_csv)
index = np.argmax(precision[2:])
index += 2

plt.plot(recall[index:], precision[index:])
plt.xlabel('recall')
plt.ylabel('precision')

plt.savefig('test_.jpg')

print('max', ':', max_predict_bbox_on_negative_images, ':', max_name)
print('min', ':', min_predict_bbox_on_positive_images, ':', min_name)

print(sorted(min_predict_bbox_on_positive_images_record))