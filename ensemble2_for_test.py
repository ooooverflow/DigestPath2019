...
def nms_exclude(dets, thresh, vote_num=2):
	"Dispatch to either CPU or GPU NMS implementations.\
	Accept dets as tensor"""
	dets = dets.cpu().detach().numpy()
	return py_cpu_nms_exclude(dets, thresh, vote_num=vote_num)

# ensemble_csv_list = []

result_dir = 'test_result_da'
csv_name = os.path.join(result_dir, 'retinanet_resnet18_round4_fold_0_weight_loss_1_on_test_data_best_valid_recall_original_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3(round3)_new.csv')

test_fold = csv_name.split('_fold_')[1][0]
print(test_fold)
ensemble_csv_list = glob.glob(csv_name)
ensemble_csv_list.append(os.path.join(result_dir, 'retinanet_resnet18_round4_fold_0_weight_loss_1_on_test_data_best_valid_recall_original_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3_new(round3).csv'))

test_fold = ensemble_csv_list[0].split('_fold_')[1][0]
...
def get_info(pred_csv, box_dict, score_dict, keep=False):
		...
		for line in lines:
			...
			if len(line[1]) != 0:
				...
				for pred in preds:
					pred = pred.split(' ')
					box = []
					if float(pred[-1]) >= 0.2 and keep == False:
						continue
...
for i, ensemble_csv in enumerate(ensemble_csv_list):
	if i == 1:
		...
		keep = False
	pred_dict_box, pred_dict_score = get_info(ensemble_csv, pred_dict_box, pred_dict_score, keep=keep)
...
for image_name in result_dict:
	...
	scores = score_dict[image_name]
	for i, result in enumerate(results):
		box = result
		for element in box:
			result_str += str(element)
			result_str += ' '
		result_str += '{}'.format(scores[i])
		result_str += ';'
	result_str += '\n'

result_csv = './test_result_da/retinanet_resnet18_round4_fold_0_weight_loss_1_on_test_data_best_valid_recall_ensemble(test)_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3(round3).csv'
