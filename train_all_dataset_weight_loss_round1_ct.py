...
def main(params):
	...
	description = 'resnet18_round2_1e-4_no_pretrain_remove_some_da_cooperative_training_0.5(round0)_0.5_multi2(round1)'

	if params['writer'] == True:
		writer = SummaryWriter(comment=
'_4fold_{}_{}_all_dataset(pos+neg)_weight_loss_{}_{}'.format(params['test_fold'], params['cooperative'], params['weight_loss'], description))
	...
	# warm up at first 10 epoch, after 10 will be ReduceLROnPlateau
	warmup_iters = 10
	scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=3, verbose=True, factor=0.9)

	# scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=params['max_epoch'])
	if params['warmup'] == True:
		warmup_scheduler = WarmupMultiStepLR(optimizer, warmup_iters=warmup_iters)
	...
	for epoch in range(params['max_epoch']):
		...
		if epoch < warmup_iters and params['warmup'] == True:
...
'ckpt_new/latest_fold_{}_{}_all_dataset_weight_loss_{}_{}.pth'.format(params['test_fold'], params['cooperative'], params['weight_loss'], description))
...
#
# 			if params['writer'] == True:
# 				writer.add_scalar('epoch/average froc', float(froc_record), epoch)
...
if __name__ == '__main__':
	...
	params = {
		...
		'max_epoch': 300,
		'test_fold': test_fold,
		'train_txt': '../train_test_4/train_{}_{}.txt'.format(test_fold, cooperative),
		...
		'test_stride_num': 3,
		'weight_loss': 1,
		'label_csv': 'bbox_ct/retinanet_resnet18_round2_for_traning_on_fold_0_0_weight_loss_1_0.5_ensemble_mixmatch(round0)_0.5_ensemble_multi2(round1).csv',
		'save_model': True,
		'warmup': False,
