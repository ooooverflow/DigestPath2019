...
def main(params):
    ...
    if params['writer'] == True:
        writer = SummaryWriter(comment='{}'.format(description))
    ...
    train_dataset = Ring_Cell_random_crop('../train_test/train_0.txt')
    ...
    test_dataset = Ring_Cell_all('/data/sqy/code/miccai2019/train_test/test_0.txt')
    ...
    for epoch in range(params['max_epoch']):
        ...
        torch.save(retinanet.module.state_dict(), 'ckpt/latest_{}.pth'.format(description))
        ...
        if float(average_precision) > best_ap:
            ...
            torch.save(retinanet.module.state_dict(), 'ckpt/best_ap_{}.pth'.format(description))

        if float(recall[-1]) > best_recall:
            best_recall = float(recall[-1])
            torch.save(retinanet.module.state_dict(), 'ckpt/best_recall_{}.pth'.format(description))

        if float(precision[-1]) > best_precision:
            best_precision = float(precision[-1])
            torch.save(retinanet.module.state_dict(), 'ckpt/best_precision_{}.pth'.format(description))
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'optim': 'adam',
        'max_epoch': 300,
        'train_txt': '../train_test/train_0.txt',
        'test_txt': '../train_test/test_0.txt',
        'batch_size': 8,
        'writer': True,
