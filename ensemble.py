...
csv_name = os.path.join(result_dir, 'retinanet_resnet18_round3_fold_0_weight_loss_1_on_train_data_best_valid_recall_*_0.5_ensemble(round0)_0.5_ensemble(round1)_0.6_ensemble(round2).csv')

ensemble_csv_list = glob.glob(csv_name)
ensemble_csv_list = [x for x in ensemble_csv_list if 'new' not in x]
...
test_dataset = Ring_Cell_all_dataset('/data/sqy/code/miccai2019/train_test_4/train_{}.txt'.format(0))
...
result_csv = './bbox/retinanet_resnet18_round4_fold_0_weight_loss_1_on_train_data_best_valid_recall_0.5_ensemble_mixmatch(round0)_0.5_ensemble_mixmatch(round1)_0.6_ensemble_mixmatch(round2)_{}_ensemble_mixmatch(round3).csv'.format(score_threshold)
