...
for test_fold in range(0, 4):
	result_csv = './test_result_new/retinanet_resnet18_round{}_test_fold_{}_best_valid_recall_weight_loss1.csv'.format(round, test_fold)
	if os.path.exists(result_csv):
		continue
	result_dict = {}
	test_dataset = Ring_Cell_all_dataset('/data/sqy/code/miccai2019/train_test_4/test_{}.txt'.format(test_fold))

	model_path = 'ckpt_new/best_valid_recall_resnet18_fold_{}_all_dataset_weight_loss_1_new.pth'.format(test_fold)
	...
	for i, (image, bbox, image_, image_name) in enumerate(tqdm(test_dataset)):
		...
		if len(pred_boxes) != 0:
			...
			anchors_nms_idx = nms(torch.cat([pred_boxes, pred_scores], dim=2)[0, :, :], 0.3)
