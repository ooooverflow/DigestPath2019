from dataset import Ring_Cell_random_crop, collate_fn, Ring_Cell_all_dataset, Ring_Cell_random_crop_all
import torch
from torch.utils.data import Dataset, DataLoader
import model_no_negative_focal_loss as model
import os
...
def main(params):
	...
	if params['writer'] == True:
		writer = SummaryWriter(comment='_resnet18 4fold_{} all dataset(pos+neg)'
									   ' no negative focal loss {} eval on test data'.
							   format(params['test_fold'], params['negative_focal_loss']))

	retinanet = model.resnet18(num_classes=2, pretrained=True, negative_focal_loss=params['negative_focal_loss'])
	...
	for epoch in range(params['max_epoch']):
		...
		# 		   'ckpt_new/latest_resnet18_fold_{}_all_dataset_no_negative_focal_loss_{}_eval_on_test_data.pth'.
		# 		   format(params['test_fold'], params['negative_focal_loss']))
		...
		with torch.no_grad():
			...
			# if float(recall_record[-1]) > best_recall:
			# 	best_recall = float(recall_record[-1])
			# 	best_precision = float(precision_record[-1])
				# torch.save(retinanet.module.state_dict(),
				# 		   'ckpt_new/best_recall_resnet18_fold_{}_all_dataset_no_negative_focal_loss_{}_eval_on_test_data.pth'
				# 		   .format(params['test_fold'], params['negative_focal_loss']))
			# elif float(recall_record[-1]) == best_recall:
			# 	if float(precision_record[-1]) > best_precision:
			# 		best_precision = float(precision_record[-1])
			# 		torch.save(retinanet.module.state_dict(),
			# 				   'ckpt_new/best_recall_resnet18_fold_{}_all_dataset_no_negative_focal_loss_{}_eval_on_test_data.pth'
			# 				   .format(params['test_fold'], params['negative_focal_loss']))
...
if __name__ == '__main__':
	...
	params = {
		...
		'batch_size': 32,
		'writer': False,
		'model_path': 'ckpt_new/latest_resnet18_fold_0_all_dataset_no_negative_focal_loss_0.05_eval_on_test_data.pth',
		'resume': False,
		'test_image_size': 1024,
		'test_stride_num': 3,
		'weight_loss': 1,
		'negative_focal_loss': 0.05,
		'save_model': False
	}
