...
csv_name = os.path.join(result_dir, 'retinanet_resnet18_round4_fold_2_weight_loss_1_on_test_data_best_valid_recall_*_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3(round3)_new_stride.csv')

test_fold = csv_name.split('_fold_')[1][0]
print(test_fold)
ensemble_csv_list = glob.glob(csv_name)
ensemble_csv_list = [x for x in ensemble_csv_list if 'ensemble' not in x]
...
for i, (image, bbox, image_, image_name) in enumerate(tqdm(test_dataset)):
	...
	# anchors_nms_idx = nms_exclude(torch.cat([pred_bboxs, pred_scores], dim=2)[0, :, :], nms_threshold, vote_num=2)
...
result_csv = './test_result_da/retinanet_resnet18_round4_fold_2_weight_loss_1_on_test_data_best_valid_recall_ensemble(TTA)_0.4(round0)_0.4(round1)_0.4(round2)_0.4_multi3(round3)_new_stride.csv'
