...
def main(params):
	...
	if params['writer'] == True:
		writer = SummaryWriter(comment='_{} test_4fold_{} all dataset(pos+neg) round{} on new gt;'
									   ' nms {}; score_threshold {}'.
				format(params['backbone'], params['test_fold'], params['round'], params['nms_threshold'], params['score_threshold']))
	...
	if params['backbone'] == 'resnet18':
		...
		print('no backbone named {}'.format(params['backbone']))
	...
	scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=5, verbose=True, factor=0.9)

	train_dataset = Ring_Cell_random_crop_all(params['train_txt'],
					label_csv_path=params['label_csv'])
	...
	for epoch in range(params['max_epoch']):
		...
		torch.save(retinanet.module.state_dict(),
				   'ckpt/latest_{}_test_fold_{}_all_dataset_round_{};nms_{};scores_threshold_{}.pth'
				   .format(params['backbone'], params['test_fold'], params['round'],params['nms_threshold'], params['score_threshold']))
		...
		with torch.no_grad():
			...
			if float(recall[-1]) > best_recall:
				...
				torch.save(retinanet.module.state_dict(),
						   'ckpt/best_recall_{}_test_fold_{}_all_dataset_round_{};nms_{};scores_threshold_{}.pth'
						   .format(params['backbone'], params['test_fold'], params['round'], params['nms_threshold'], params['score_threshold']))
...
if __name__ == '__main__':
	...
	params = {
		...
		'score_threshold' : 0.2,
		'backbone': 'resnet101',
		'label_csv': './bbox/retinanet_resnet_18_using_test_prediction_nms_0.4_scorethreshold_0.2_for_round2_training.csv'
	}
