...
import model_weight_loss as model
import os
...
def main(params):
	...
	if params['writer'] == True:
		writer = SummaryWriter(comment='_resnet18 4fold 0; weight loss 10')
	...
	scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=8, verbose=True, factor=0.9)
	...
	for epoch in range(params['max_epoch']):
		...
		torch.save(retinanet.module.state_dict(), 'ckpt/latest_resnet18_4fold_0_weight_loss_10.pth')
		...
		if float(average_precision) > best_ap:
			...
			torch.save(retinanet.module.state_dict(), 'ckpt/best_ap_resnet18_4fold_0_weight_10.pth')

		if float(recall[-1]) > best_recall:
			best_recall = float(recall[-1])
			torch.save(retinanet.module.state_dict(), 'ckpt/best_recall_resnet18_4fold_0_weight_10.pth')

		if float(precision[-1]) > best_precision:
			best_precision = float(precision[-1])
			torch.save(retinanet.module.state_dict(), 'ckpt/best_precision_resnet18_4fold_0_weight_10.pth')
...
if __name__ == '__main__':
	os.environ['CUDA_VISIBLE_DEVICES'] = '6, 7, 5, 2'
	params = {
		'learning_rate': 1e-4,
		'optim': 'adam',
		'max_epoch': 600,
		'train_txt': '../train_test_4/train_0.txt',
		'test_txt': '../train_test_4/test_0.txt',
		'batch_size': 8,
		'writer': True,
